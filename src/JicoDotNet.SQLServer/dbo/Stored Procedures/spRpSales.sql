CREATE PROCEDURE [dbo].[spRpSales]
(
		...
		BEGIN TRANSACTION @T1
			...
			BEGIN
				SET @SQL = 'SELECT * FROM 
						...
						FROM dbo.mInvoiceHeader as ivh
			...
			BEGIN
				SET @SQL = 'SELECT * FROM 
							...
							FROM dbo.mInvoiceDetail as ivd
							LEFT JOIN dbo.mInvoiceHeader as ivh
							...
							WHERE (ivh.InvoiceDate BETWEEN CONVERT(DATE, ''' + CAST(YEAR(@StartDate) as VARCHAR) + '-' 
								...
								IIF(@ForRetail IS NULL, 
									' ', 
									' AND ivh.CustomerId IN (SELECT CustomerId FROM dbo.mCustomer 
