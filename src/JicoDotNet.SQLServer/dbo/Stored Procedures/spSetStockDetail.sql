...
CREATE PROCEDURE [dbo].[spSetStockDetail]
(
		...
		BEGIN TRANSACTION @T1
			...
			BEGIN
				...
				BEGIN
					...
					IF EXISTS(SELECT * FROM dbo.mProduct 
					...
					BEGIN
						...
						SELECT @StockDetailId = StockDetailId, 
								...
								@PrevBatchNo = BatchNo
						FROM dbo.tStockDetail
						WHERE WareHouseId = @WareHouseId 
							AND ProductId = @ProductId;
						-- Add New Row for every Perishable Products
						IF((SELECT IsPerishableProduct FROM dbo.mProduct WHERE ProductId = @ProductId) = 1)
						BEGIN
							PRINT(3.1);
						...
						*/
						...
						BEGIN
							...
							VALUES
								...
								@ExpiryDate, @BatchNo, @Remarks, @GRNDate, 1, dbo.ISTNow(), @RequestId);
				...
				BEGIN
					...
					SELECT @GRNDate = GRNDate FROM dbo.tStockDetail WHERE StockDetailId = @StockDetailId;
					EXEC [dbo].[spSetStockDetail]
						@StockDetailId = 0,
						@StockId = @StockId,
						@ProductId = @ProductId,
